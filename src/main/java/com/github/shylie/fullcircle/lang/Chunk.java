...
public class Chunk {
    ...
    public int dissasembleInstruction(StringBuilder log, int offset) {
        ...
        switch (instruction) {
            ...
            case OpCode.LOAD_3:
                return argInstruction(log, "OP_LOAD_3", offset);

            case OpCode.STORE:
                return argInstruction(log, "OP_STORE", offset);

            case OpCode.STORE_3:
                return argInstruction(log, "OP_STORE_3", offset);
    ...
    private int simpleInstruction(StringBuilder log, String name, int offset) {
        writeLog(log, "%-20s %04d", name, offset);
        return offset + 1;
    }
    ...
    private int constantInstruction(StringBuilder log, String name, int offset) {
        ...
        writeLog(log, "%-20s %04d '%s'", name, offset, constant < constants.size() ? constants.get(constant) : null);
        return offset + 2;
    }

    private int argInstruction(StringBuilder log, String name, int offset) {
        writeLog(log, "%-20s %04d %04d", name, offset, code.get(offset + 1));
        return offset + 2;
    }
