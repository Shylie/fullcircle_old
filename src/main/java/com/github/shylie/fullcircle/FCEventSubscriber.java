...
public class FCEventSubscriber {
	private static Set<UUID> noCastSpell = new HashSet<>();
	...
	public static void onPlayerUseItem(PlayerInteractEvent.RightClickItem event) {
		...
		if (noCastSpell.contains(event.getPlayer().getUniqueID())) {
			noCastSpell.remove(event.getPlayer().getUniqueID());
			return;
		}
	...
	public static void onPlayerInteract(PlayerInteractEvent.RightClickBlock event) {
		...
		if (block == Blocks.LIME_GLAZED_TERRACOTTA && item == Items.BOOK && event.getItemStack().getCount() == 1) {
			...
			noCastSpell.add(event.getPlayer().getUniqueID());
		}
